import { o as e } from "./chunk-CMxvf4Kt.js";
import { n as t, t as n } from "./jsx-runtime-D0Gpvp3q.js";
import { t as r } from "./x-CZ5V7eis.js";
import { i, o as a, t as o } from "./utils-BUTip_f_.js";
import { t as s } from "./dist-D9ZlzrYt.js";
import { i as c, n as l, r as u, t as d } from "./dist-Cc8O5o4E.js";
import { t as f } from "./dist-DTO6IMEA.js";
import {
  a as ee,
  i as p,
  n as m,
  o as h,
  r as g,
  s as te,
  t as _,
} from "./es2015-CtULDcww.js";
var v = e(t(), 1),
  y = n(),
  b = `Dialog`,
  [x, S] = f(b),
  [C, w] = x(b),
  T = (e) => {
    let {
        __scopeDialog: t,
        children: n,
        open: r,
        defaultOpen: i,
        onOpenChange: a,
        modal: o = !0,
      } = e,
      s = v.useRef(null),
      c = v.useRef(null),
      [d, f] = u({ prop: r, defaultProp: i ?? !1, onChange: a, caller: b });
    return (0, y.jsx)(C, {
      scope: t,
      triggerRef: s,
      contentRef: c,
      contentId: l(),
      titleId: l(),
      descriptionId: l(),
      open: d,
      onOpenChange: f,
      onOpenToggle: v.useCallback(() => f((e) => !e), [f]),
      modal: o,
      children: n,
    });
  };
T.displayName = b;
var E = `DialogTrigger`,
  D = v.forwardRef((e, t) => {
    let { __scopeDialog: n, ...r } = e,
      i = w(E, n),
      o = a(t, i.triggerRef);
    return (0, y.jsx)(s.button, {
      type: `button`,
      "aria-haspopup": `dialog`,
      "aria-expanded": i.open,
      "aria-controls": i.open ? i.contentId : void 0,
      "data-state": G(i.open),
      ...r,
      ref: o,
      onClick: c(e.onClick, i.onOpenToggle),
    });
  });
D.displayName = E;
var O = `DialogPortal`,
  [k, A] = x(O, { forceMount: void 0 }),
  j = (e) => {
    let { __scopeDialog: t, forceMount: n, children: r, container: i } = e,
      a = w(O, t);
    return (0, y.jsx)(k, {
      scope: t,
      forceMount: n,
      children: v.Children.map(r, (e) =>
        (0, y.jsx)(d, {
          present: n || a.open,
          children: (0, y.jsx)(p, { asChild: !0, container: i, children: e }),
        }),
      ),
    });
  };
j.displayName = O;
var M = `DialogOverlay`,
  N = v.forwardRef((e, t) => {
    let n = A(M, e.__scopeDialog),
      { forceMount: r = n.forceMount, ...i } = e,
      a = w(M, e.__scopeDialog);
    return a.modal
      ? (0, y.jsx)(d, {
          present: r || a.open,
          children: (0, y.jsx)(F, { ...i, ref: t }),
        })
      : null;
  });
N.displayName = M;
var P = i(`DialogOverlay.RemoveScroll`),
  F = v.forwardRef((e, t) => {
    let { __scopeDialog: n, ...r } = e,
      i = w(M, n),
      o = a(t, te());
    return (0, y.jsx)(m, {
      as: P,
      allowPinchZoom: !0,
      shards: [i.contentRef],
      children: (0, y.jsx)(s.div, {
        "data-state": G(i.open),
        ...r,
        ref: o,
        style: { pointerEvents: `auto`, ...r.style },
      }),
    });
  }),
  I = `DialogContent`,
  L = v.forwardRef((e, t) => {
    let n = A(I, e.__scopeDialog),
      { forceMount: r = n.forceMount, ...i } = e,
      a = w(I, e.__scopeDialog);
    return (0, y.jsx)(d, {
      present: r || a.open,
      children: a.modal
        ? (0, y.jsx)(ne, { ...i, ref: t })
        : (0, y.jsx)(re, { ...i, ref: t }),
    });
  });
L.displayName = I;
var ne = v.forwardRef((e, t) => {
    let n = w(I, e.__scopeDialog),
      r = v.useRef(null),
      i = a(t, n.contentRef, r);
    return (
      v.useEffect(() => {
        let e = r.current;
        if (e) return _(e);
      }, []),
      (0, y.jsx)(R, {
        ...e,
        ref: i,
        trapFocus: n.open,
        disableOutsidePointerEvents: n.open,
        onCloseAutoFocus: c(e.onCloseAutoFocus, (e) => {
          (e.preventDefault(), n.triggerRef.current?.focus());
        }),
        onPointerDownOutside: c(e.onPointerDownOutside, (e) => {
          let t = e.detail.originalEvent,
            n = t.button === 0 && t.ctrlKey === !0;
          (t.button === 2 || n) && e.preventDefault();
        }),
        onFocusOutside: c(e.onFocusOutside, (e) => e.preventDefault()),
      })
    );
  }),
  re = v.forwardRef((e, t) => {
    let n = w(I, e.__scopeDialog),
      r = v.useRef(!1),
      i = v.useRef(!1);
    return (0, y.jsx)(R, {
      ...e,
      ref: t,
      trapFocus: !1,
      disableOutsidePointerEvents: !1,
      onCloseAutoFocus: (t) => {
        (e.onCloseAutoFocus?.(t),
          t.defaultPrevented ||
            (r.current || n.triggerRef.current?.focus(), t.preventDefault()),
          (r.current = !1),
          (i.current = !1));
      },
      onInteractOutside: (t) => {
        (e.onInteractOutside?.(t),
          t.defaultPrevented ||
            ((r.current = !0),
            t.detail.originalEvent.type === `pointerdown` && (i.current = !0)));
        let a = t.target;
        (n.triggerRef.current?.contains(a) && t.preventDefault(),
          t.detail.originalEvent.type === `focusin` &&
            i.current &&
            t.preventDefault());
      },
    });
  }),
  R = v.forwardRef((e, t) => {
    let {
        __scopeDialog: n,
        trapFocus: r,
        onOpenAutoFocus: i,
        onCloseAutoFocus: a,
        ...o
      } = e,
      s = w(I, n);
    return (
      g(),
      (0, y.jsx)(y.Fragment, {
        children: (0, y.jsx)(ee, {
          asChild: !0,
          loop: !0,
          trapped: r,
          onMountAutoFocus: i,
          onUnmountAutoFocus: a,
          children: (0, y.jsx)(h, {
            role: `dialog`,
            id: s.contentId,
            "aria-describedby": s.descriptionId,
            "aria-labelledby": s.titleId,
            "data-state": G(s.open),
            ...o,
            ref: t,
            deferPointerDownOutside: !0,
            onDismiss: () => s.onOpenChange(!1),
          }),
        }),
      })
    );
  }),
  z = `DialogTitle`,
  B = v.forwardRef((e, t) => {
    let { __scopeDialog: n, ...r } = e,
      i = w(z, n);
    return (0, y.jsx)(s.h2, { id: i.titleId, ...r, ref: t });
  });
B.displayName = z;
var V = `DialogDescription`,
  H = v.forwardRef((e, t) => {
    let { __scopeDialog: n, ...r } = e,
      i = w(V, n);
    return (0, y.jsx)(s.p, { id: i.descriptionId, ...r, ref: t });
  });
H.displayName = V;
var U = `DialogClose`,
  W = v.forwardRef((e, t) => {
    let { __scopeDialog: n, ...r } = e,
      i = w(U, n);
    return (0, y.jsx)(s.button, {
      type: `button`,
      ...r,
      ref: t,
      onClick: c(e.onClick, () => i.onOpenChange(!1)),
    });
  });
W.displayName = U;
function G(e) {
  return e ? `open` : `closed`;
}
var K = T,
  q = j,
  J = v.forwardRef(({ className: e, ...t }, n) =>
    (0, y.jsx)(N, {
      ref: n,
      className: o(
        `fixed inset-0 z-50 bg-black/80  data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0`,
        e,
      ),
      ...t,
    }),
  );
J.displayName = N.displayName;
var Y = v.forwardRef(({ className: e, children: t, ...n }, i) =>
  (0, y.jsxs)(q, {
    children: [
      (0, y.jsx)(J, {}),
      (0, y.jsxs)(L, {
        ref: i,
        className: o(
          `fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 sm:rounded-lg`,
          e,
        ),
        ...n,
        children: [
          t,
          (0, y.jsxs)(W, {
            className: `absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background cursor-pointer transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground`,
            children: [
              (0, y.jsx)(r, { className: `h-4 w-4` }),
              (0, y.jsx)(`span`, { className: `sr-only`, children: `Close` }),
            ],
          }),
        ],
      }),
    ],
  }),
);
Y.displayName = L.displayName;
var X = ({ className: e, ...t }) =>
  (0, y.jsx)(`div`, {
    className: o(`flex flex-col space-y-1.5 text-center sm:text-left`, e),
    ...t,
  });
X.displayName = `DialogHeader`;
var Z = ({ className: e, ...t }) =>
  (0, y.jsx)(`div`, {
    className: o(
      `flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2`,
      e,
    ),
    ...t,
  });
Z.displayName = `DialogFooter`;
var Q = v.forwardRef(({ className: e, ...t }, n) =>
  (0, y.jsx)(B, {
    ref: n,
    className: o(`text-lg font-semibold leading-none tracking-tight`, e),
    ...t,
  }),
);
Q.displayName = B.displayName;
var $ = v.forwardRef(({ className: e, ...t }, n) =>
  (0, y.jsx)(H, {
    ref: n,
    className: o(`text-sm text-muted-foreground`, e),
    ...t,
  }),
);
$.displayName = H.displayName;
export {
  X as a,
  W as c,
  N as d,
  j as f,
  S as h,
  Z as i,
  L as l,
  D as m,
  Y as n,
  Q as o,
  B as p,
  $ as r,
  T as s,
  K as t,
  H as u,
};
