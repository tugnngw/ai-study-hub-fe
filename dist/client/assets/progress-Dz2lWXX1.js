import { o as e } from "./chunk-CMxvf4Kt.js";
import { n as t, t as n } from "./jsx-runtime-D0Gpvp3q.js";
import { t as r } from "./createLucideIcon-p5NglK5C.js";
import { t as i } from "./utils-BUTip_f_.js";
import { t as a } from "./dist-D9ZlzrYt.js";
import { t as o } from "./dist-DTO6IMEA.js";
var s = r(`database`, [
    [`ellipse`, { cx: `12`, cy: `5`, rx: `9`, ry: `3`, key: `msslwz` }],
    [`path`, { d: `M3 5V19A9 3 0 0 0 21 19V5`, key: `1wlel7` }],
    [`path`, { d: `M3 12A9 3 0 0 0 21 12`, key: `mv7ke4` }],
  ]),
  c = e(t(), 1),
  l = n(),
  u = `Progress`,
  d = 100,
  [f, p] = o(u),
  [m, h] = f(u),
  g = c.forwardRef((e, t) => {
    let {
      __scopeProgress: n,
      value: r = null,
      max: i,
      getValueLabel: o = y,
      ...s
    } = e;
    (i || i === 0) && !S(i) && console.error(w(`${i}`, `Progress`));
    let c = S(i) ? i : d;
    r !== null && !C(r, c) && console.error(T(`${r}`, `Progress`));
    let u = C(r, c) ? r : null,
      f = x(u) ? o(u, c) : void 0;
    return (0, l.jsx)(m, {
      scope: n,
      value: u,
      max: c,
      children: (0, l.jsx)(a.div, {
        "aria-valuemax": c,
        "aria-valuemin": 0,
        "aria-valuenow": x(u) ? u : void 0,
        "aria-valuetext": f,
        role: `progressbar`,
        "data-state": b(u, c),
        "data-value": u ?? void 0,
        "data-max": c,
        ...s,
        ref: t,
      }),
    });
  });
g.displayName = u;
var _ = `ProgressIndicator`,
  v = c.forwardRef((e, t) => {
    let { __scopeProgress: n, ...r } = e,
      i = h(_, n);
    return (0, l.jsx)(a.div, {
      "data-state": b(i.value, i.max),
      "data-value": i.value ?? void 0,
      "data-max": i.max,
      ...r,
      ref: t,
    });
  });
v.displayName = _;
function y(e, t) {
  return `${Math.round((e / t) * 100)}%`;
}
function b(e, t) {
  return e == null ? `indeterminate` : e === t ? `complete` : `loading`;
}
function x(e) {
  return typeof e == `number`;
}
function S(e) {
  return x(e) && !isNaN(e) && e > 0;
}
function C(e, t) {
  return x(e) && !isNaN(e) && e <= t && e >= 0;
}
function w(e, t) {
  return `Invalid prop \`max\` of value \`${e}\` supplied to \`${t}\`. Only numbers greater than 0 are valid max values. Defaulting to \`${d}\`.`;
}
function T(e, t) {
  return `Invalid prop \`value\` of value \`${e}\` supplied to \`${t}\`. The \`value\` prop must be:
  - a positive number
  - less than the value passed to \`max\` (or ${d} if no \`max\` prop is set)
  - \`null\` or \`undefined\` if the progress is indeterminate.

Defaulting to \`null\`.`;
}
var E = g,
  D = v,
  O = c.forwardRef(({ className: e, value: t, ...n }, r) =>
    (0, l.jsx)(E, {
      ref: r,
      className: i(
        `relative h-2 w-full overflow-hidden rounded-full bg-primary/20`,
        e,
      ),
      ...n,
      children: (0, l.jsx)(D, {
        className: `h-full w-full flex-1 bg-primary transition-all`,
        style: { transform: `translateX(-${100 - (t || 0)}%)` },
      }),
    }),
  );
O.displayName = E.displayName;
export { s as n, O as t };
