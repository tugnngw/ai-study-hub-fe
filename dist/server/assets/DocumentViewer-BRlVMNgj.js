...
import { ChevronLeft, ChevronRight, Download, ExternalLink, File, Hand, Loader2, Maximize2, MousePointer2, RotateCw, ZoomIn, ZoomOut } from "lucide-react";
...
var PdfViewer = ({ url, fileName = "document.pdf", className, documentId }) => {
	...
	useEffect(() => {
		...
		pdfDoc.getPage(pageNumber).then((page) => {
			const fitScale = containerWidth / page.getViewport({ scale: 1 }).width * .95;
			setScale(Math.min(Math.max(fitScale, .25), 3));
		});
	}, [pdfDoc, pageNumber]);
	const handleZoom = (direction) => {
		if (direction === "fit") handleFitToWidth();
		else if (direction === "in") setScale((prev) => Math.min(prev + .25, 3));
		else setScale((prev) => Math.max(prev - .25, .25));
	};
	const handleFitToWidth = async () => {
		if (!pdfDoc || !containerRef.current) return;
		const vp = (await pdfDoc.getPage(pageNumber)).getViewport({ scale: 1 });
		const containerWidth = containerRef.current.clientWidth - 64;
		setScale(Math.min(Math.max(containerWidth / vp.width * .95, .25), 3));
	};
	useEffect(() => {
		...
		const handleWheel = (e) => {
			if (e.ctrlKey || e.metaKey) {
				...
				setScale((prev) => Math.min(Math.max(prev + delta, .25), 3));
			}
		};
	...
	const handleMouseDown = (e) => {
		...
		if (containerRef.current) setScrollStart({
	...
	useEffect(() => {
		const handleKeyDown = (e) => {
			if (!numPages) return;
			if ((e.key === "ArrowLeft" || e.key === "PageUp") && !e.ctrlKey && !e.metaKey) setPageNumber((p) => Math.max(1, p - 1));
			else if ((e.key === "ArrowRight" || e.key === "PageDown") && !e.ctrlKey && !e.metaKey) setPageNumber((p) => Math.min(numPages, p + 1));
			else if (e.key === "Home") setPageNumber(1);
			else if (e.key === "End") setPageNumber(numPages);
		};
		window.addEventListener("keydown", handleKeyDown);
		return () => window.removeEventListener("keydown", handleKeyDown);
	}, [numPages]);
	const Toolbar = /* @__PURE__ */ jsxs("div", {
		className: "flex items-center justify-between px-4 py-2 border-b border-border bg-muted/30 shrink-0 flex-wrap gap-1",
		children: [/* @__PURE__ */ jsxs("div", {
			...
			children: [
				/* @__PURE__ */ jsx(Button, {
					...
					children: isPanMode ? /* @__PURE__ */ jsx(Hand, { className: "h-4 w-4" }) : /* @__PURE__ */ jsx(MousePointer2, { className: "h-4 w-4" })
				}),
				...
				/* @__PURE__ */ jsx(Input, {
					...
					onChange: (e) => {
						setScale(Math.min(Math.max(Number(e.target.value) || 25, 25), 300) / 100);
					},
				...
				/* @__PURE__ */ jsxs("div", {
					...
					children: [/* @__PURE__ */ jsx(Input, {
						...
						value: pageNumber,
						onChange: (e) => {
							setPageNumber(Math.min(Math.max(parseInt(e.target.value) || 1, 1), numPages || 1));
						},
				...
				/* @__PURE__ */ jsx(Button, {
					...
					children: /* @__PURE__ */ jsxs("a", {
						...
						children: [/* @__PURE__ */ jsx(Download, { className: "h-4 w-4 mr-1" }), "Tải"]
					})
				}),
				/* @__PURE__ */ jsx(Button, {
					variant: "ghost",
	...
	return /* @__PURE__ */ jsxs(Card, {
		...
		children: [Toolbar, /* @__PURE__ */ jsx("div", {
			ref: containerRef,
			className: cn("flex-1 overflow-auto bg-muted/40 flex items-start justify-center py-6", isPanMode && "cursor-grab", dragging && "cursor-grabbing"),
			...
			children: loading ? /* @__PURE__ */ jsxs("div", {
				...
				children: [/* @__PURE__ */ jsx(Loader2, { className: "h-10 w-10 animate-spin text-primary mb-3" }), /* @__PURE__ */ jsx("p", {
				...
				children: [/* @__PURE__ */ jsx("canvas", {
					...
					className: "absolute bottom-2 right-2 bg-black/60 text-white text-[10px] px-2 py-0.5 rounded-full",
					children: [
						pageNumber,
						"/",
						numPages
