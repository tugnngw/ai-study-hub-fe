...
import { t as adminDocumentApi } from "./documentApi-CSfvEboD.js";
import { t as FilePreviewDialog } from "./FilePreviewDialog-q5aU2687.js";
...
var useAdminDocuments = (tab) => {
	...
	const ts = (/* @__PURE__ */ new Date()).toISOString().slice(11, 23);
	console.log(`[RQ:${ts}] useQuery RENDER key=${JSON.stringify(key)} observerCount=${queryInfo.observers?.length ?? "?"} fetchStatus=${queryInfo.fetchStatus} status=${queryInfo.status} isStale=${queryInfo.isStale} isInvalidated=${queryInfo.isInvalidated} isFetching=${queryInfo.isFetching} dataCount=${queryInfo.data?.length ?? 0}`);
	return queryInfo;
};
