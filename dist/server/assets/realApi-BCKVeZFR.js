import { n as api, r as refreshTokens } from "./api-BmsV1BBR.js";
...
var authApi = {
	...
	requestPasswordReset: (email) => api("/api/auth/forgot-password", {
		method: "POST",
		body: { email }
	}),
	...
	resetPassword: (email, otp, newPassword) => api("/api/auth/reset-password", {
		method: "POST",
		body: {
			email,
			otp,
	...
	verifyEmail: (token) => api(`/api/auth/verify?token=${encodeURIComponent(token)}`, { method: "POST" }),
	sendVerification: (email) => api(`/api/auth/send-verification?email=${encodeURIComponent(email)}`, { method: "POST" }),
	resendVerificationByUsername: (username) => api(`/api/auth/resend-verification-by-username?username=${encodeURIComponent(username)}`, { method: "POST" }),
	resendVerification: () => api("/api/auth/resend-verification", { method: "POST" })
};
var accountApi = {
	me: () => api("/api/account/me"),
	updateProfile: (body) => api("/api/account/profile", {
		method: "PUT",
		body
	}),
	changePassword: (body) => api("/api/account/password", {
		method: "PUT",
		body
	})
};
var semesterApi = {
	...
	create: (name) => api("/api/semesters", {
		method: "POST",
		body: { name }
	}),
	delete: (id) => api(`/api/semesters/${id}`, { method: "DELETE" })
};
var subjectApi = {
	listAll: () => api("/api/subjects"),
	listBySemester: (semesterId) => api(`/api/subjects/semester/${semesterId}`),
	getById: (id) => api(`/api/subjects/${id}`),
	create: (body) => api("/api/subjects", {
		method: "POST",
		body
	}),
	delete: (id) => api(`/api/subjects/${id}`, { method: "DELETE" })
};
var folderApi = {
	...
	listTrash: () => api("/api/folder/trash"),
	restoreFromTrash: (id) => api(`/api/folder/${id}/restore`, { method: "POST" }),
	permanentDelete: (id) => api(`/api/folder/${id}/permanent`, { method: "DELETE" })
};
var documentApi = {
	getUploadConfig: () => api("/api/documents/upload-config"),
...
var shareApi = {
	...
	submitAppeal: (body) => api("/api/reports/appeal", {
		method: "POST",
		body: {
			documentId: body.id,
			reason: body.reason,
...
var quizApi = {
	...
	submit: (quizId, answers) => api(`/api/quizzes/${quizId}/submit`, {
		method: "POST",
		body: { answers }
	})
};
