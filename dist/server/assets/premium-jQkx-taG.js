...
import { t as paymentApi } from "./paymentApi-C09ju49W.js";
...
function PremiumUpgradePage() {
	...
	useEffect(() => {
		const init = async () => {
			try {
				await reloadUser();
				const [plansData, userData] = await Promise.all([paymentApi.getPlanOptions(), accountApi.me()]);
				setPlans(plansData);
	...
	useEffect(() => {
		const refreshUser = async () => {
			try {
				...
				if (userData?.plan && userData.storageGb) setCurrentPlan(userData.plan.toUpperCase());
			} catch (error) {
