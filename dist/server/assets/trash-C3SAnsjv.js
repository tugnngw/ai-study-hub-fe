...
import { t as adminUserApi } from "./userApi-C4_lQcuD.js";
import { t as adminDocumentApi } from "./documentApi-CPUF0HW7.js";
import { t as reportApi } from "./reportApi-KcG1Lvh3.js";
	...
	getDeletedAccounts: async () => {
		try {
			return (await adminUserApi.getTrashUsers()).map((a) => ({
				...
				remainingDays: 30 - Math.floor((Date.now() - new Date(a.deletedAt ?? /* @__PURE__ */ new Date()).getTime()) / (1e3 * 60 * 60 * 24))
			}));
		} catch {
			return [];
		}
