...
var OpenXmlPackage = class OpenXmlPackage {
	...
	static async load(input, options) {
		return new OpenXmlPackage(await JSZip.loadAsync(input), options);
	}
	save(type = "blob") {
...
async function renderDocument(document, bodyContainer, styleContainer, userOptions) {
	...
	return await new HtmlRenderer(window.document).render(document, bodyContainer, styleContainer, ops);
}
