...
import { j as useShareFolder, y as useOwnedShares } from "./queries-BaPhj2xN.js";
...
function ShareEntityDialog({ open, onOpenChange, title, folderId, documentId }) {
	...
	const matchingShare = (shares ?? []).find((s) => isFolder ? s.folderId === targetId : s.documentId === targetId);
	const shareToken = share.data?.shareToken ?? matchingShare?.shareToken ?? "";
	const link = shareToken ? `${window.location.origin}/shared/${shareToken}` : "";
