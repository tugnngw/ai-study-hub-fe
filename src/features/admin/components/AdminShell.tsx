    ...
    <div className="min-h-screen flex">
      ...
      >
        <div className={cn("px-5 py-5 border-b border-border flex items-center gap-2", collapsed ? "justify-center px-0" : "")}>
          <Link to="/admin_panel" className="flex items-center gap-2.5 group min-w-0">
            ...
            {!collapsed && (
              <div className="min-w-0">
                ...
                <div className="text-[10px] text-muted-foreground tracking-wider uppercase">Quản trị</div>
              </div>
            )}
          </Link>
          {!collapsed && (
              ...
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 hover:bg-accent rounded-full pr-3 pl-1 py-1 transition-colors">
                  ...
                  <div className="hidden sm:flex flex-col items-start leading-tight">
                    <span className="text-xs font-medium">{user?.fullName ?? "Admin"}</span>
                    <span className="text-[10px] text-muted-foreground">@{user?.username ?? "admin"}</span>
                  </div>
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-60">
                <DropdownMenuLabel className="pb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="font-semibold truncate">{user?.fullName ?? "Quản trị viên"}</div>
                      <div className="text-xs text-muted-foreground font-normal truncate">{user?.email ?? "admin"}</div>
                    </div>
                  </div>
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
