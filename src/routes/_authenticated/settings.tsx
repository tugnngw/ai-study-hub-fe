      ...
      <div className="rounded-2xl border border-border/70 bg-card p-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 rounded-full bg-gradient-brand text-white flex items-center justify-center text-[16px] font-bold">
              {initial}
            </div>
            <div>
              ...
              <p className="text-muted-foreground text-[12px]">
                Thành viên từ{" "}
                {(user as any)?.createdAt?.slice(0, 10) ?? "—"}
              </p>
            </div>
          </div>
          <button
            ...
            className="px-4 py-2 rounded-lg border border-border text-[13px] font-medium hover:bg-secondary/60"
          >
            Chỉnh sửa thông tin
          </button>
        </div>
        ...
        <div className="divide-y divide-border/60">
          {rows.map((r) => (
            <button
              ...
              type="button"
              onClick={r.onClick}
              className="w-full flex items-center gap-4 py-4 text-left hover:bg-secondary/30 -mx-2 px-2 rounded-lg transition-colors"
            >
...
function ChangePasswordDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-card rounded-2xl p-6 w-full max-w-[420px] shadow-2xl">
        ...
        <div className="flex justify-end items-center gap-3 mt-6">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2 rounded-lg border border-border text-[14px] font-medium hover:bg-secondary/60"
          >
            Hủy
          </button>
          <button
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full h-11 rounded-xl border border-input bg-background px-3.5 text-[14px] outline-none focus:border-primary"
      />
    </div>
  );
}
