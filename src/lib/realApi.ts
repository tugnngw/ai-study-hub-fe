...
export const folderApi = {
  ...
  getById: (id: string): Promise<Folder> => api<Folder>(`/api/folder/getbyid/${id}`),
  ...
  listTrash: (): Promise<Folder[]> => api<Folder[]>("/api/folder/trash"),
  restoreFromTrash: (id: string): Promise<void> =>
    api<void>(`/api/folder/${id}/restore`, { method: "POST" }),
  permanentDelete: (id: string): Promise<void> =>
    api<void>(`/api/folder/${id}/permanent`, { method: "DELETE" }),
};
...
export const documentApi = {
  ...
  getById: (id: string): Promise<Document> => api<Document>(`/api/documents/${id}`),
  getSharedById: (id: string): Promise<Document> =>
    api<Document>(`/api/documents/shared/${id}`),

  // POST /api/documents (multipart/form-data) — backend expects files[] array
  ...
  listTrash: (): Promise<Document[]> => api<Document[]>("/api/documents/trash"),
  restoreFromTrash: (id: string): Promise<void> =>
...
export const ragApi = {
  ...
  // Returns Map<String,String> = { documentId, status }
  status: (documentId: string): Promise<{ documentId: string; status: string }> =>
  ...
  chat: (input: AskRequest): Promise<AskResponse> =>
...
export const shareApi = {
  ...
  // POST /api/shares — backend handles both folderId and documentId
  ...
  saveToMyFolder: (
    ...
    title?: string,
    description?: string,
  ): Promise<import("./types").SaveToFolderResponse> =>
    api<import("./types").SaveToFolderResponse>(`/api/shares/${shareId}/save`, {
      method: "POST",
  ...
  listMyReports: (): Promise<import("./types").ReportResponse[]> =>
    api<import("./types").ReportResponse[]>("/api/reports/my"),
};
...
export const quizApi = {
  ...
  generate: (input: import("./types").GenerateQuizRequest): Promise<QuizResponse> =>
...
export const flashcardApi = {
  ...
  generate: (input: GenerateFlashcardsRequest): Promise<GenerateFlashcardsResponse> =>
    api<GenerateFlashcardsResponse>("/api/flashcards/generate", {
      method: "POST",
      body: input,
    }),
};
...
export const quotaApi = {
  getDetails: (): Promise<QuotaDetails> => api<QuotaDetails>("/api/quota"),
};
